...
import me.jrwang.aloha.common.{AlohaConf, Logging, config}


/**
  * Continuously appends the data from an input stream into the given file.
  */
private[aloha] class FileAppender(inputStream: InputStream, file: File, bufferSize: Int = 8192)
  extends Logging {
...
/**
  * Companion object to [[FileAppender]] which has helper
  * functions to choose the correct type of FileAppender based on AlohaConf configuration.
  */
private[aloha] object FileAppender extends Logging {

  /** Create the right appender based on Aloha configuration */
  def apply(inputStream: InputStream, file: File, conf: AlohaConf): FileAppender = {
