...
private object JavaDeserializationStream {
  val primitiveMappings: Map[String, Class[_]] = Map[String, Class[_]](
...
/**
  * :: DeveloperApi ::
  * A Aloha serializer that uses Java's built-in serialization.
  *
  * @note This serializer is not guaranteed to be wire-compatible across different versions of
  * Aloha. It is intended to be used to serialize/de-serialize data within a single
  * Aloha application.
  */
class JavaSerializer(conf: AlohaConf) extends Serializer with Externalizable {
  ...
  protected def this() = this(new AlohaConf())  // For deserialization only
