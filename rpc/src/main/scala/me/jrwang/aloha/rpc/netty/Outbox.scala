...
import me.jrwang.aloha.rpc.{RpcAddress, RpcEnvStoppedException, RpcException}
import me.jrwang.aloha.transport.client.{RpcResponseCallback, TransportClient}
import me.jrwang.aloha.common.{AlohaException, Logging}
...
private[netty] case class OneWayOutboxMessage(content: ByteBuf) extends OutboxMessage with Logging {
  ...
  override def onFailure(e: Throwable): Unit = {
    e match {
      ...
      case e1: Throwable => logError(s"Failed to send one-way RPC.", e1)
    }
  }
}
...
private[netty] case class RpcOutboxMessage(
    ...
    _onSuccess: (TransportClient, ByteBuf) => Unit)
...
private[netty] class Outbox(nettyEnv: NettyRpcEnv, val address: RpcAddress) {
  ...
  /**
    ...
    * [[Outbox]] is stopped, the sender will be notified with a [[RpcException]].
    */
  ...
  def stop(): Unit = {
    ...
    var message = messages.poll()
    while (message != null) {
      message.onFailure(new AlohaException("Message is dropped because Outbox is stopped"))
      message = messages.poll()
    }
  }
}